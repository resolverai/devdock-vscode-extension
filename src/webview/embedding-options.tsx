...
export const EmbeddingOptions = () => {
  ...
  const { context: rerankThreshold = 0.5, setContext: setRerankThreshold } =
    useGlobalContext<number>(EXTENSION_CONTEXT_NAME.devdockRerankThreshold)

  const { context: maxChunkSize = '500', setContext: setMaxChunkSize } =
    useGlobalContext<string>(EXTENSION_CONTEXT_NAME.devdockMaxChunkSize)

  const { context: minChunkSize = '50', setContext: setMinChunkSize } =
    useGlobalContext<string>(EXTENSION_CONTEXT_NAME.devdockMinChunkSize)

  const { context: overlap = '20', setContext: setOverlap } =
    useGlobalContext<string>(EXTENSION_CONTEXT_NAME.devdockOverlapSize)

  const { context: codeSnippets = '5', setContext: setRelevantCodeSnippets } =
    useGlobalContext<string>(EXTENSION_CONTEXT_NAME.devdockRelevantCodeSnippets)

  const { context: filePaths = '10', setContext: setRelevantFilePaths } =
    useGlobalContext<string>(EXTENSION_CONTEXT_NAME.devdockRelevantFilePaths)

  const { context: metric = 'l2', setContext: setMetric } =
    useGlobalContext<string>(EXTENSION_CONTEXT_NAME.devdockVectorSearchMetric)

  const embeddingProviders = Object.values(getProvidersByType('embedding'))

  const handleEmbedDocuments = () => {
    global.vscode.postMessage({
      type: EVENT_NAME.devdockEmbedDocuments
