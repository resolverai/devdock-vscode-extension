...
import {
  ...
  getProvider,
  updateMessagesForDevDockProvider,
} from "./utils";
import {
...
export class ConversationHistory {
  ...
  streamConversationTitle({
    ...
    const provider = getProvider(this._context); //this.getProvider();
  ...
  private buildStreamRequest(messages?: Message[]) {
    const provider = getProvider(this._context);
  ...
  async getConversationTitle(messages: Message[]): Promise<string> {
    const myMessage = updateMessagesForDevDockProvider(messages);
  ...
  async saveConversation(conversation: Conversation) {
    ...
    if (
      this._sessionManager.get(EXTENSION_SESSION_NAME.devdockSymmetryConnection)
    ) {
