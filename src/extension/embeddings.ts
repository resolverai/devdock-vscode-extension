...
import { getDocumentSplitChunks, getProvider } from "./utils";
...
export class EmbeddingDatabase {
  ...
  private getProvider = () =>
    this._extensionContext?.globalState.get<DevdockProvider>(
      ACTIVE_EMBEDDINGS_PROVIDER_STORAGE_KEY
    );

  public async fetchModelEmbedding(content: string) {
    const provider = getProvider(this._extensionContext);
