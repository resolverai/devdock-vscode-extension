...
export const getTerminal = async (): Promise<Terminal | undefined> => {
  const devdockTerminal = window.terminals.find((t) => t.name === TWINNY);
...
export const getChunkOptions = (
  ...
  const maxChunkSizeContext = `${EVENT_NAME.devdockGlobalContext}-${EXTENSION_CONTEXT_NAME.devdockMaxChunkSize}`;
  const minChunkSizeContext = `${EVENT_NAME.devdockGlobalContext}-${EXTENSION_CONTEXT_NAME.devdockMinChunkSize}`;
  const overlap = `${EVENT_NAME.devdockGlobalContext}-${EXTENSION_CONTEXT_NAME.devdockOverlapSize}`;
...
export const updateSymmetryStatus = (
  ...
  view?.webview.postMessage({
    type: EVENT_NAME.devdockSendSymmetryMessage,
    value: {
      data: message,
    },
  } as ServerMessage<string>);
...
export const logStreamOptions = (opts: any) => {
  logger.log(
    `
    ****Devdock opts**** ${JSON.stringify(opts)} \n\
