...
export class SymmetryService extends EventEmitter {
  ...
  constructor(
    ...
    const autoConnectProvider = this._context.globalState.get(
      `${EVENT_NAME.devdockGlobalContext}-${GLOBAL_STORAGE_KEY.autoConnectSymmetryProvider}`
    )
  ...
  public disconnect = async () => {
    ...
    this.view?.webview.postMessage({
      type: EVENT_NAME.devdockDisconnectedFromSymmetry
    } as ServerMessage)
  }
  ...
  public connectToProvider = async (connection: SymmetryConnection) => {
    ...
    this._providerSwarm.on('connection', (peer: any) => {
      ...
      this.view?.webview.postMessage({
        type: EVENT_NAME.devdockConnectedToSymmetry,
        value: {
          data: {
  ...
  private createOrUpdateProviderConfig(providerConfig: DevdockProvider): void {
  ...
  public async startSymmetryProvider() {
    ...
    const sessionKey = EXTENSION_SESSION_NAME.devdockSymmetryConnectionProvider

    this._sessionManager?.set(sessionKey, 'connecting')

    const sessionTypeName = `${EVENT_NAME.devdockSessionContext}-${sessionKey}`
